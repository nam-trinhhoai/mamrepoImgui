#include "qtimgui_demo.h"

DemoMultipleWindows::DemoMultipleWindows(const QString& name, int time, WorkingSetManager* manager):
...
void DemoMultipleWindows::showPlot() {
	...
	if(totalNumberOfWellBores > 0)
	{
		...
		// Plot
		{
			...
			//ImPlot::SetNextPlotLimits(logMin, logMax, depthMin, depthMax, ImGuiCond_Always);
			//if (ImPlot::BeginPlot(logName.c_str(), logName.c_str(), "Depth", ImVec2(-1,-1), 0, 0, ImPlotAxisFlags_Invert)) {
