#include "QtImGuiPlotMultipleSeismicExtractionsAndWellLogs.h"


QtImGuiPlotMultipleSeismicExtractionsAndWellLogs::QtImGuiPlotMultipleSeismicExtractionsAndWellLogs(const QString& name, int time, WorkingSetManager* manager):
QtImGuiCore(name, time),
m_manager(manager)
{}

QtImGuiPlotMultipleSeismicExtractionsAndWellLogs::~QtImGuiPlotMultipleSeismicExtractionsAndWellLogs(){}


// Seismic extraction.
std::pair<bool, QtImGuiPlotMultipleSeismicExtractionsAndWellLogs::IJKPoint> QtImGuiPlotMultipleSeismicExtractionsAndWellLogs::isPointInBoundingBox(Seismic3DAbstractDataset* dataset, WellUnit wellUnit, double logKey, WellBore* wellBore) 
{
	IJKPoint pt = {0,0,0};
	bool out = false;
...
// Interactive helper		
int QtImGuiPlotMultipleSeismicExtractionsAndWellLogs::interactiveHelper(double* depth)
{
...
void QtImGuiPlotMultipleSeismicExtractionsAndWellLogs::longCrossHairCursor()
{
	int x, y;
	Uint32 buttons;
...
// Drag and drop to plot a log
void QtImGuiPlotMultipleSeismicExtractionsAndWellLogs::showPlot() {
	...
	{		
		...
		// Plot
		{
			...
			static ImPlotRect lims(0,1,depthMax,depthMin);
						...
						if(numSeismics >0)
						{
							for (int k = 0; k < numSeismics; ++k) {
								if ((dnd_seismic[k].Plt == 1) & (dnd_seismic[k].chartIdx == pltIdx)) {
									...
									// Update data range
									for(int i=0; i<seismic_extract.size(); i++)
									{
										if(seismic_extract[i] != nullValue)
										{
											if(seismic_extract[i] < logMin)
												logMin = seismic_extract[i];
											if(seismic_extract[i] > logMax)
												logMax = seismic_extract[i];
										}

										if(depth_seismic_update[i] < depthMin)
											depthMin = depth_seismic_update[i];
										if(depth_seismic_update[i] > depthMax)
											depthMax = depth_seismic_update[i];
									}
